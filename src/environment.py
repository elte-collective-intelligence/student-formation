...
def draw(self):
    ...
    # determine the scaling factor for the camera view in the environment
...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world, np_random):
        ...
        for agent in world.agents:
            if MOUNTAINS:
                # state.p_pos => the position of the agent in the world. p_pos stands for "position" 
                # and is usually a vector indicating the agent's coordinates in the world.

                # in [CIRCLE_CENTER[i] - CIRCLE_RADIUS,CIRCLE_CENTER[i] + CIRCLE_RADIUS], init position of agent 
            ...
            #velocity of agent is set to 0
            agent.state.p_vel = np.zeros(world.dim_p)
            #communication state is set to 0
            agent.state.c = np.zeros(world.dim_c)
        #landmark.boundary is True, it likely means this landmark serves as a boundary or a wall in the environment.
        for i, landmark in enumerate(world.landmarks):
            if not landmark.boundary:
                #By placing the landmarks randomly within the specified range (-0.9 to +0.9
                # ), it allows for a diverse setup in the environment where agents can interact with
                # different landmarks at the start of a simulation.
                landmark.state.p_pos = np_random.uniform(-0.9, +0.9, world.dim_p)
                #Setting the velocity to zero means that these landmarks will not move on their own,
                #allowing agents to interact with them without needing to account for moving obstacles.
    ...
    def reward(self, agent, world):
        ...
        def bound(x):
            """
            Calculate boundary penalty for agents.
            """
            if x < 0.9:
                return 0
            if x < 1.0:
